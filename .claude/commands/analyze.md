...
```bash
# Build the grep pattern from Phase 1 findings
PATTERN="product.?family|product.?pillar"  # adjust to request

rg -i "$PATTERN" metadata/ --type xml -o --no-filename \
  | grep -oE '[A-Za-z][A-Za-z0-9_]*__c' \
  | sort -u > cache/discovered-fields.txt

cat cache/discovered-fields.txt
```
Report: "Found [N] matching fields: [list]"

### Phase 4 — Scan all metadata types
Use `bash scripts/scan.sh` with the discovered fields, or run targeted
rg commands per the scan matrix in CLAUDE.md.

For every match: read the file content, extract the specific usage.
Do not list filenames only — explain what each automation actually does.

### Phase 5 — Runtime SOQL
Run the standard set from CLAUDE.md Phase 5.
Add any specific queries the request requires.
